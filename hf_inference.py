...
# def load_prompt_string_from_json(prompt_path_str: str, n_sample = None, return_list=True):
#     if not os.path.isfile(prompt_path_str):
#         raise FileNotFoundError(
#             f'{prompt_path_str=} does not match any existing files.')
#     if prompt_path_str.endswith('json'):
#         dh = DataHandler(test_file_path=prompt_path_str)
#         raw_datasets = dh.load_data(extension='json', task='ate')
#         prompt_df = raw_datasets['test'].to_pandas()
    
#     if n_sample is None:
#         n_sample = len(prompt_df)

#     if return_list:
#         return prompt_df[input_col].to_list()[:n_sample]
#     else:
#         return prompt_df[:n_sample]
    

def load_prompt_string_from_csv(prompt_path_str: str, n_sample = None, return_list=True):
    if not os.path.isfile(prompt_path_str):
        raise FileNotFoundError(
            f'{prompt_path_str} does not match any existing files.')
    if prompt_path_str.endswith('csv'):
        prompt_df = pd.read_csv(prompt_path_str)
...
def main(args: Namespace) -> None:
    ...
    if args.input_data_path.endswith('csv'):
        prompt_strings = load_prompt_string_from_csv(args.input_data_path, args.n_sample)
    if args.input_data_path.endswith('json'):
        prompt_strings = load_prompt_string_from_json(args.input_data_path, args.n_sample)
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.name_or_path, **from_pretrained_kwargs)
    ...
    for temp, topp, topk, repp, nrnz, seed in itertools.product(
        ...
        for batch in tqdm(batches, desc="Inference"):
            ...
            encoded_inp = tokenizer(batch, return_tensors='pt', padding=True, truncation=True, max_length=2048)
            for key, value in encoded_inp.items():
                #value = torch.where(value < 2048, value, torch.tensor(2))
            ...
            if args.warmup and (not done_warmup):
                print('Warming up...')
                # print(encoded_inp['input_ids'])
                # maxi = torch.max(encoded_inp['input_ids'])
                # print(maxi)
                # encoded_inp['input_ids'] = torch.where(encoded_inp['input_ids'] < 2048, encoded_inp['input_ids'], torch.tensor(2))
                # maxi = torch.max(encoded_inp['input_ids'])
                # print(maxi)
            ...
            for idx, (effective_prompt, prompt, gen) in enumerate(
                ...
                if len(continuation) > 0:
                    ...
                    outputs.append(full_generation)
        ...
        if args.output_data_path and args.input_data_path:
            if args.input_data_path.endswith('csv'):
                out_df = load_prompt_string_from_csv(args.input_data_path, args.n_sample, return_list=False)
            # if args.input_data_path.endswith('json'):
            #     out_df = load_prompt_string_from_json(args.input_data_path, args.n_sample, return_list=False)
            out_df['preds'] = outputs
            out_df.to_csv(args.output_data_path, index=False)
